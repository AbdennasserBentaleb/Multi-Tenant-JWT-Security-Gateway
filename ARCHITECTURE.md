...
4. An application-level filter (`TenantContextFilter`) stores the `tenant_id` in a Java 21 `ThreadLocal` context.
...
### 3. Context Propagation via `ThreadLocal`

While new paradigms like `ScopedValue` exist in newer Java versions, this project implements a traditional and well-understood `ThreadLocal` approach to context propagation. The `TenantContextFilter` carefully sets and cleans up the thread state per-request, ensuring there are no memory leaks or context spillage across HTTP requests.
